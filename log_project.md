...
## 2026-06-20 — Fix Pantip screenshot Chromium binary tracing on Vercel

### Problem
- Testing `/dashboard/pantip` preview on Vercel failed with:
...
- Root cause: the `@sparticuz/chromium` runtime code was available, but the required Brotli binary files in `node_modules/@sparticuz/chromium/bin` were not included in the Vercel/Next.js serverless output trace.

### Changes
- Updated `next.config.ts`:
  - Added `serverExternalPackages` for `@sparticuz/chromium` and `puppeteer-core`.
  - Added `outputFileTracingIncludes` for `/api/pantip/preview` to include `./node_modules/@sparticuz/chromium/bin/**/*`.
- Updated `src/lib/pantip/screenshot.ts`:
  - Added an explicit check for the bundled Chromium bin folder.
  - Calls `chromium.executablePath(bundledChromiumBinPath)` when the bin folder exists.
  - Keeps `PUPPETEER_EXECUTABLE_PATH` override for local debugging.

### Database / Storage
- No database changes.
- No storage added.
- Pantip screenshots remain temporary and are posted directly to Facebook only after user review.

### Next steps
- Redeploy to Vercel because `next.config.ts` affects production build output.
- Test `/dashboard/pantip` preview again on production.

---

## 2026-06-20 — Improve Pantip preview diagnostics and speed up screenshot loading

### Problem
- After fixing Chromium binary tracing, `/dashboard/pantip` still showed only `Request failed` on preview creation.
- The UI error was too generic because non-JSON Vercel/server error responses were discarded by the client.
- Pantip pages can load many third-party resources, so waiting for full network idle can be slow or cause Vercel/serverless failures.

### Changes
- Updated `src/app/dashboard/pantip/pantip-source-client.tsx`:
  - Reads raw response text before parsing JSON.
  - Shows HTTP status and a short server response preview when the API returns non-JSON errors.
  - This should make future Vercel errors visible on the screen instead of only `Request failed`.
- Updated `src/app/api/pantip/preview/route.ts`:
  - Added stage-based runtime logs: `start`, `workspace`, `screenshot`, `caption`, `success`.
  - Adds `stage` and `elapsedMs` to technical errors returned from the API.
- Updated `src/lib/pantip/screenshot.ts`:
  - Changed navigation wait strategy from `networkidle2` to `domcontentloaded` to avoid waiting for ads/tracking/long-running requests.
  - Blocks non-essential image/media/font and common third-party tracking/ad requests during screenshot generation.
  - Reduced screenshot width/height and JPEG quality slightly to keep response payload smaller.
- Updated `vercel.json`:
  - Added function max duration entries for the Pantip preview/publish route handlers as an explicit Vercel fallback.
...
- No new tables.
- No storage added.
- Pantip screenshot images are still temporary only.

### Next steps
- Redeploy to Vercel.
- Test `/dashboard/pantip` again.
- If it still fails, copy the full new error text from the UI and check Vercel Runtime Logs for `[pantip-preview]` stage logs.

---

## 2026-06-21 — Improve Pantip screenshot detail and human-style caption controls

### Context
- User tested `/dashboard/pantip` successfully: the app generated both the preview image and the AI caption.
- Two issues remained:
  - The preview image showed mostly Pantip layout/background and did not clearly display useful thread details.
  - The generated caption was too long and sounded too much like a bot/article summary.

### Changes
- Updated `src/lib/pantip/screenshot.ts`:
  - Stopped blocking normal image requests, while still blocking media/fonts and common tracking/ad URLs.
  - Waits for body text to render before capture.
  - Extracts topic title/excerpt from Pantip meta tags and tries to locate the rendered topic title on the page.
  - Captures around the rendered title area when found, instead of always cropping from the very top of the page.
  - Adds a fallback generated source card if the headless browser does not render the Pantip topic content clearly, so the preview image is not blank or mostly empty.
  - Still does not scroll down into comments and still hides comment-like elements.
- Updated `src/app/api/pantip/preview/route.ts`:
  - Accepts optional `styleInstructions` from the Pantip UI and passes it to Gemini.
  - Trims the style note to 800 characters to keep the prompt controlled.
- Updated `src/lib/ai/gemini.ts`:
  - Tightened Pantip caption prompt to be shorter and more human-like.
  - Reduced Pantip-specific default max length to 70–90 words, while still respecting the main Writing Style.
  - Added explicit banned wording for bot-like/formal summaries such as “บทเรียน”, “ประเด็นที่น่าสนใจ”, “สถานการณ์”, “สะท้อนให้เห็น”, and similar phrasing.
  - Uses “อ่านต้นทาง:” for the final source link line.
- Updated `src/app/dashboard/pantip/pantip-source-client.tsx`:
  - Added a “สไตล์แคปชั่นรอบนี้” textarea so the user can guide the Pantip caption style before generating the preview.
  - Explains that this note works together with the main `/dashboard/style` Writing Style.
  - Adds a hint near the generated caption telling the user to adjust the style note and regenerate if the caption still sounds like a bot.
- Updated `README.md`:
  - Documented the optional Pantip-specific style note.
  - Documented the non-storage fallback source card behavior.
...
- No storage added.
- Screenshot images remain temporary only and are uploaded directly to Facebook when the user manually approves posting.

### Environment Variables
- No new environment variables.

### Expected behavior after deploy
- `/dashboard/pantip` should show a more useful image preview.
- If Pantip does not render topic details in the serverless browser, the preview should fall back to a readable generated card using the topic title/excerpt instead of showing a mostly blank Pantip page.
- Pantip captions should be shorter and closer to a human “เจอกระทู้นี้แล้วเอามาเล่าเอง” style.
- The user can adjust the style note before clicking “สร้างตัวอย่างโพสต์”.

### Next steps
- Redeploy to Vercel.
- Test the same Pantip link again.
- If the preview still does not show the desired visual style, decide whether to keep improving true screenshot capture or intentionally use the generated source card as the stable image format.

### Validation in assistant environment
- `npm install --no-audit --no-fund` passed.
- `npm run lint -- --no-cache` passed.
- `npx tsc --noEmit` passed.
- `npm run build` compiled successfully and finished TypeScript, then timed out in the sandbox while Next.js was collecting page data with workers. This sandbox timeout has happened before; user should still run `npm run build` locally or rely on Vercel production build for final confirmation.

---

## 2026-06-21 — Switch Pantip preview screenshot to mobile viewport

### Context
- User noticed that a Pantip screenshot might look better if captured like a mobile browser because most Facebook viewers read posts on phones.
- The previous Pantip preview captured a wider desktop-like view, which could waste space and make the topic details less readable.

### Changes
- Updated `src/lib/pantip/screenshot.ts`:
  - Replaced the desktop-like viewport with a mobile viewport (`430 x 932`, device scale factor `2`).
  - Switched the Pantip browser user agent to an iPhone Safari style user agent.
  - Adjusted title capture offset for a mobile screen.
  - Simplified screenshot capture to use the visible mobile viewport instead of a desktop clip rectangle.
  - Resized the fallback generated source card to fit the mobile screenshot format.
  - Kept the existing behavior of not scrolling down into comments, hiding comment-like sections, not storing screenshots, and keeping the fallback card if Pantip does not render clearly.
- Updated `src/app/dashboard/pantip/pantip-source-client.tsx`:
  - Clarified the UI text that the preview is captured in a mobile-screen style.
  - Constrained the preview image display width so it visually matches the mobile capture.
- Updated `README.md`:
  - Documented the mobile screenshot behavior for Pantip Source Post.
...
### Expected behavior after deploy
- `/dashboard/pantip` should show a tall mobile-style preview image.
- The preview should be more suitable for Facebook mobile viewers.
- The system should still avoid comments and should still require manual user approval before posting.


---

## 2026-06-21 — Pantip readable-card fallback and shorter captions

### Context
- User tested the mobile Pantip preview and found that the real screenshot still rendered incomplete, showing a mostly blue/placeholder page without useful topic details.
- User also wanted the generated caption to look like the extracted “ตัวอย่างข้อความสั้น ๆ” instead of a long bot-like explanation.

### Changes
- Updated `src/lib/pantip/screenshot.ts`:
  - Added a visible viewport quality check after scrolling to the presumed topic header.
  - If the visible mobile screenshot does not contain the topic title, has too little Thai text, or looks mostly numeric/placeholder-like, the system switches to the readable generated source card.
  - Improved the fallback card design to look more like a mobile Pantip-style source preview with title, short excerpt, and source link.
  - Added an `imageMode` value so the UI can tell whether the preview is a real Pantip screenshot or a readable card fallback.
  - Screenshot images are still temporary only and are not stored in the app/database/storage.
- Updated `src/app/api/pantip/preview/route.ts`:
  - Returned `imageMode` from the preview API.
- Updated `src/app/dashboard/pantip/pantip-source-client.tsx`:
  - Added `imageMode` to the preview response type.
  - Changed the default Pantip style instruction to prefer using the extracted short excerpt as the caption base.
  - Reduced the caption textarea height because captions should now be shorter.
  - Added UI text explaining when a readable card fallback was used.
- Updated `src/lib/ai/gemini.ts`:
  - Rewrote the Pantip caption prompt to strongly prefer a very short caption based on the extracted excerpt.
  - Added post-processing that falls back to the extracted excerpt plus source link if Gemini returns a caption that is too long or too formal/bot-like.
- Updated `README.md`:
  - Documented the readable-card fallback and short caption behavior.
...
- No database schema changes.
- No storage added.
- Screenshots remain temporary and are uploaded directly to Facebook only after manual user approval.

### Environment Variables
- No new environment variables.

### Expected behavior after deploy
- If Pantip renders correctly in headless Chromium, the system can still use the mobile screenshot.
- If Pantip renders as an incomplete/placeholder page, the system should automatically show a readable source card instead of a broken-looking screenshot.
- Caption should be much shorter and closer to the extracted “ตัวอย่างข้อความสั้น ๆ”, followed by the original Pantip link.

### Validation in assistant environment
- `npm install --no-audit --no-fund --registry=https://registry.npmjs.org/` passed.
- `npm run lint -- --no-cache` passed.
- `npx tsc --noEmit` passed.
- `npm run build` compiled successfully and finished TypeScript, then timed out in the sandbox while Next.js was collecting page data with workers. User should run `npm run build` locally or rely on Vercel production build for final confirmation.


---

## 2026-06-21 — Pantip readable card as the default preview image

### Context
- User concluded that the live Pantip webpage screenshot was still unreliable and often rendered as an incomplete placeholder/blue layout without readable topic details.
- User agreed to switch fully to the readable-card approach instead of trying to keep a real Pantip screenshot as the main image.

### Changes
- Updated `src/lib/pantip/screenshot.ts`:
  - Removed dependence on taking a live Pantip webpage screenshot as the primary preview image.
  - Added server-side metadata extraction from the Pantip topic HTML using `fetch`, reading the title and short excerpt from meta tags / page content.
  - Kept the temporary image workflow, but now the preview image is always a generated readable card built from the Pantip title, short excerpt, and source link.
  - Still uses headless Chromium only to render the generated readable-card HTML into a temporary JPEG for Facebook posting.
  - `imageMode` is now always `readable_card` for this flow.
- Updated `src/app/dashboard/pantip/pantip-source-client.tsx`:
  - Changed helper text so the UI now explains that the system creates a readable card instead of a live webpage screenshot.
  - Adjusted the preview description to match the new behavior.
- Updated `README.md`:
  - Documented that Pantip preview images now use the readable-card approach by default.
...
- No database schema changes.
- No storage added.
- Preview images remain temporary only and are uploaded directly to Facebook only after manual approval.

### Environment Variables
- No new environment variables.

### Expected behavior after deploy
- `/dashboard/pantip` should always generate a readable preview card.
- The preview image should always have readable topic details instead of a broken Pantip webpage render.
- Caption flow remains manual-review-first, and posting still uses the same Facebook Page token / Gemini configuration as before.


---

## 2026-06-21 — Pantip readable-card text cleanup

### Context
- User tested the readable-card approach and confirmed it was no longer a real Pantip screenshot.
- The card image still looked wrong because it included Pantip layout/navigation/footer text such as `Pantip Download App` and `Pantip Certified Developer` instead of the real topic details.
- The right-side extracted title/excerpt/caption were closer to the desired content, so the fix focuses on cleaning and choosing better metadata before rendering the card.

### Changes
- Updated `src/lib/pantip/screenshot.ts`:
  - Added stronger Pantip metadata candidate selection for title and excerpt from `<title>`, Open Graph/Twitter meta tags, heading tags, and embedded JSON-like values.
  - Added cleanup/rejection rules for layout/noise text such as Pantip app download links, certified developer text, navigation labels, URLs, and numeric-heavy placeholder strings.
  - Made the readable card always use sanitized `title` and `excerpt`; if the excerpt is not useful, it falls back to the title rather than using footer/menu text.
  - Simplified and improved the readable-card design so the topic title and short excerpt are larger and clearer.
- Updated `src/app/dashboard/pantip/pantip-source-client.tsx`:
  - Renamed the preview label to “Preview การ์ดกระทู้”.
  - Clarified that the card is generated from title/excerpt/link and does not use the real Pantip page screenshot.
- Updated `README.md`:
  - Documented the text cleanup behavior for Pantip readable cards.
...
- No database schema changes.
- No storage added.
- Preview images remain temporary and are uploaded directly to Facebook only after manual approval.

### Environment Variables
- No new environment variables.

### Expected behavior after deploy
- The Pantip preview card should no longer show layout/footer text such as `Pantip Download App` or `Pantip Certified Developer`.
- The card should show a readable Thai topic title, a short excerpt, and the Pantip source link.
- The Pantip feature remains fully manual: paste one link, create preview, review, and manually post.


---

## 2026-06-21 — Pantip caption starts with full topic title

### Context
- User confirmed the AI caption quality is mostly appropriate, but wanted the caption to start by pulling the full Pantip topic title directly, then add a short 1-2 sentence summary or discussion angle.
- This should make short-title topics less abrupt while preserving the source topic title clearly.

### Changes
- Updated `src/lib/ai/gemini.ts`:
  - Changed the Pantip teaser prompt so paragraph 1 must be the full topic title as provided.
  - Paragraph 2 should now add a short 1-2 sentence summary or discussion angle based on the extracted excerpt.
  - Added post-processing to ensure the final caption starts with the topic title before appending the source link.
  - Updated fallback caption generation to use `title + short summary/excerpt + source link` instead of excerpt-only text.
- Updated `src/app/dashboard/pantip/pantip-source-client.tsx`:
  - Changed the default per-run style instruction to request full-title-first captions.
...
### Expected behavior after deploy
- Pantip AI caption should start with the full topic title.
- The second paragraph should add a short 1-2 sentence summary or discussion angle.
- The caption should still end with `อ่านต้นทาง: <Pantip URL>`.


---

## 2026-06-21 — Pantip card Thai font rendering and title/detail card

### Context
- User confirmed that Pantip title extraction and AI caption are working acceptably.
- The remaining issue is the generated preview card image: English letters/numbers appeared but Thai text was missing, which indicates a likely server-side Thai font rendering issue in headless Chromium.
- User requested that the card be built from the topic title and the topic detail text only for now.

### Changes
- Updated `src/lib/pantip/screenshot.ts`:
  - Kept the readable-card approach; the renderer does not screenshot the live Pantip page.
  - Changed the card design to a 1080x1350 Facebook-friendly vertical card.
  - Added a Thai web font (`Noto Sans Thai`) to the generated card HTML and waits for `document.fonts.ready` before taking the image screenshot.
  - The card now uses the extracted topic title and extracted topic detail/excerpt as the primary visual content.
  - Added more aggressive text-candidate extraction from visible HTML blocks and quoted Thai strings, while still filtering Pantip menu/footer/layout text.
- Updated `src/app/dashboard/pantip/pantip-source-client.tsx`:
  - Changed UI wording from “short excerpt” to “topic detail text” for the card preview/debug display.
- Updated `README.md`:
  - Documented that the generated card uses Thai font loading and title/detail content.
...
- No database schema changes.
- No storage added.
- Card images remain temporary and are uploaded directly to Facebook only after manual approval.

### Environment Variables
- No new environment variables.

### Expected behavior after deploy
- The preview card should render Thai text correctly in the image.
- The card should show the extracted topic title and topic detail/excerpt.
- The card should not contain Pantip menu/footer text.


---

## 2026-06-21 — Pantip caption empty-response fallback

### Context
- User tested the Pantip preview after the Thai font/card patch and hit an error at `stage=caption`: `Gemini returned empty Pantip teaser`.
- The preview should not fail entirely when Gemini returns an empty response, because the system already has the Pantip title/excerpt needed for a safe fallback caption.

### Changes
- Updated `src/lib/ai/gemini.ts`:
  - If Gemini returns an empty Pantip teaser, the system now falls back to a deterministic caption built from the extracted topic title, topic detail/excerpt, and source URL.
  - The same fallback is also used when Gemini output is too long or too bot-like.
  - This prevents `/dashboard/pantip` preview generation from failing solely because Gemini returned an empty caption.
...
### Expected behavior after deploy
- If Gemini returns an empty Pantip caption, preview generation should still succeed.
- The fallback caption should use the format: full topic title, short detail/summary paragraph, and `อ่านต้นทาง: <Pantip URL>`.


---

## 2026-06-21 — News Source Post MVP from RSS

### Context
- User wanted a new semi-automatic news posting workflow, separate from Auto Pilot and Pantip.
- Required behavior: fetch RSS news, show a list, let the user choose one story, let AI summarize in Thai using the existing Writing Style, no image/card, always credit source and link, preview first, and manually click publish.
- User clarified that AI may read/translate the available full article text for understanding, but must not copy or translate the full article into the Facebook post.

### Changes
- Added `src/lib/news/rss.ts`:
  - Defines starter RSS sources for world news and football/news categories.
  - Supports custom RSS URL input.
  - Parses RSS/Atom feeds with built-in TypeScript helpers, without adding new dependencies.
  - Fetches article page text when possible and falls back to RSS title/summary when full article content is unavailable.
- Added `src/app/api/news/items/route.ts`:
  - Authenticated endpoint to load RSS items for a built-in or custom source.
- Added `src/app/api/news/preview/route.ts`:
  - Authenticated endpoint to fetch article content and generate a Thai caption preview with Gemini using the existing Writing Profile.
- Added `src/app/api/news/publish/route.ts`:
  - Authenticated endpoint to publish the reviewed text-only caption to the connected Facebook Page.
- Added `src/app/dashboard/news/page.tsx` and `src/app/dashboard/news/news-source-client.tsx`:
  - New Dashboard page `/dashboard/news`.
  - User selects a source, loads items, picks a story, generates a preview, edits the caption, then manually publishes.
  - The UI explicitly states that the flow is manual-only, uses no card/image, and requires source credit/link.
- Updated `src/lib/ai/gemini.ts`:
  - Added `generateNewsSourcePostWithGemini`.
  - Prompt writes short Thai news summaries in the user's existing page style, like explaining to a friend.
  - Blocks bot-like lead-ins and requires `ที่มา:` plus `อ่านต่อ:` lines.
  - Falls back to a safe headline/summary/source/link caption if Gemini returns empty or too formal.
- Updated `src/components/dashboard/dashboard-nav.tsx`:
  - Added the new `News Post` menu item.
- Updated `README.md` with the new News Source Post behavior.

### Files changed
- `src/lib/news/rss.ts`
- `src/lib/ai/gemini.ts`
- `src/app/api/news/items/route.ts`
- `src/app/api/news/preview/route.ts`
- `src/app/api/news/publish/route.ts`
- `src/app/dashboard/news/page.tsx`
- `src/app/dashboard/news/news-source-client.tsx`
- `src/components/dashboard/dashboard-nav.tsx`
- `README.md`
- `log_project.md`
...
- No database schema changes.
- No storage added.
- News source list is hardcoded for MVP plus a custom RSS URL option.

### Environment Variables
- No new environment variables.
- Uses existing `GEMINI_API_KEY`, `GEMINI_MODEL`, and connected Facebook Page settings.

### Expected behavior after deploy
- `/dashboard/news` should load RSS items from the selected source.
- User can choose one item and generate a Thai caption preview.
- Caption should use existing page Writing Style, feel like explaining news to a friend, and include source credit plus original link.
- User must manually click publish; no Auto Pilot behavior is added.


---

## 2026-06-21 — Add category-specific RSS sources for News Source Post

### Context
- User confirmed the News Source Post MVP can load RSS items.
- User asked whether news organizations have RSS feeds by category, not only one combined feed.
- Decision: keep the feature manual-only and add more hardcoded category-specific RSS sources for easier testing before adding any database-backed source management.

### Changes
- Updated `src/lib/news/rss.ts`:
  - Added `NewsCategory` and `NEWS_CATEGORY_LABELS`.
  - Expanded built-in RSS sources across starter categories: World, Business/Economy, Technology, Science, Health, Entertainment/Culture, Travel, Sport, and Football.
  - Added more BBC, Guardian, and CNN feed entries while keeping Custom RSS URL support.
- Updated `src/app/dashboard/news/news-source-client.tsx`:
  - Source selector now groups feeds by Thai category labels using `<optgroup>`.
  - Updated helper text to explain that the selected source is now category-specific.
  - News item header now shows category + source name after loading.
- Updated `README.md`:
  - Documented the expanded category-specific RSS source behavior.
...
- No database schema changes.
- No storage added.
- Source list remains hardcoded for MVP plus custom RSS input.

### Environment Variables
- No new environment variables.

### Expected behavior after deploy
- `/dashboard/news` should show the source dropdown grouped by category.
- User can choose a more specific source such as BBC Business, BBC Technology, Guardian Science, CNN Travel, or Football feeds.
- Some external feeds may fail or return no items depending on the publisher; Custom RSS URL remains available for testing alternatives.


---

## 2026-06-21 — News Source Post writing modes and natural lead-ins

### Context
- User liked the News Source Post direction but felt that news posts should not always be very short.
- Decision: allow the user to choose between writing modes: short summary, fuller rewritten news post, and sports/football style.
- User also requested avoiding bot-like openings such as “CNN รายงานว่า...” or “มีรายงานว่า...”.

### Changes
- Updated `src/lib/ai/gemini.ts`:
  - Added `NewsPostMode` with `story`, `short`, and `sports`.
  - Added mode-specific prompt settings for length, paragraph count, and tone.
  - Default mode is `story` (`เล่าเป็นข่าวแบบเต็มขึ้น`).
  - Prompt now explicitly forbids bot-like lead-ins including `CNN รายงานว่า`, `BBC รายงานว่า`, `The Guardian รายงานว่า`, `มีรายงานว่า`, and `สำนักข่าวรายงานว่า`.
  - Added post-processing to strip common bot-like source lead-ins if Gemini still starts with them.
  - Fallback caption now respects the selected mode.
- Updated `src/app/api/news/preview/route.ts`:
  - Accepts `postMode` from the client and passes it to Gemini.
  - Returns `postMode` in the preview payload.
- Updated `src/app/dashboard/news/news-source-client.tsx`:
  - Added a `รูปแบบโพสต์` selector with three choices: fuller story, short summary, and sports/football.
  - Updated default style instruction to avoid source-name lead-ins and bot-like phrasing.
- Updated `README.md` with the writing mode behavior.
...
### Expected behavior after deploy
- `/dashboard/news` should let the user choose the writing mode before generating a preview.
- Captions should start with the news angle directly rather than `CNN รายงานว่า...` or `มีรายงานว่า...`.
- The flow remains manual-only: load RSS, choose an item, preview/edit caption, and manually publish.


---

## 2026-06-21 — Pantip card uses strict beginning-of-topic body

### Context
- User found that some Pantip cards correctly showed the topic body, but other cards pulled comment text or mid-topic text instead.
- Hypothesis: the previous candidate scoring could choose a high-scoring comment or middle section when the original topic body was short or long.
- Decision: the card must use the beginning of the original topic body only. If that text is short, keep it short. If the topic body is long, truncate from the beginning. Do not fall back to comments.

### Changes
- Updated `src/lib/pantip/screenshot.ts`:
  - Added comment markers such as `ความคิดเห็น`, `ความคิดเห็นที่`, `แสดงความคิดเห็น`, `ตอบกลับ`, `ถูกใจ`, `สมาชิกหมายเลข`, and English comment/reply markers.
  - Added strict topic-body-start extraction anchored after the selected topic title.
  - The extractor now cuts off text before comment markers and rejects comment-like candidates.
  - Fallback excerpt selection is limited to metadata description candidates only; visible text / JSON candidate scoring is no longer used as a broad fallback for card body text.
  - Logs now include whether strict topic body extraction was used.
- Updated `src/app/dashboard/pantip/pantip-source-client.tsx`:
  - Updated helper text to clarify that the card uses beginning-of-topic text and should not use comments.
- Updated `src/lib/ai/gemini.ts`:
  - Removed the unused `modeSettings` variable from the News fallback caption helper to clear the existing lint warning.
- Updated `README.md` with the strict Pantip topic-body behavior.
...
### Expected behavior after deploy
- Pantip cards should show the title and the beginning of the original topic body.
- If a topic body is short, the card keeps the short body instead of pulling a comment.
- If a topic body is long, the card uses the beginning of the body and truncates it.
- Comments should not appear in the card detail text.


---

## 2026-06-21 — Pantip extractor prioritizes `display-post-story`

### Context
- User found that some Pantip cards still pulled layout/menu text or comment text instead of the original topic body.
- User identified Pantip's topic body container as `div.display-post-story`.

### Changes
- Updated `src/lib/pantip/screenshot.ts`:
  - Added explicit extraction from `div.display-post-story` in fetched HTML.
  - Added explicit extraction from `.display-post-story` in browser-rendered DOM payloads.
  - Prioritizes `display-post-story` text before older fallback methods.
  - Cuts text after comment markers when validating topic-body candidates.
  - Keeps safe fallback behavior rather than using comments or Pantip menu/footer text as the card detail.
...
### Expected behavior after deploy
- Pantip card detail should prefer the original topic body from `display-post-story`.
- Short topics should stay short instead of falling back to comments.
- Long topics should use the beginning of the topic body instead of the middle of the page or comments.


---

## 2026-06-21 — Pantip main story wrapper selector

### Context
- User found the DOM difference between the main Pantip topic body and comments:
  - Main topic text is under `.display-post-story-wrapper`.
  - Comment text can be under `.display-post-wrapper-inner`.
- Previous selector used `.display-post-story`, which Pantip also uses in comments, so some cards could still show comment text.

### Changes
- Updated `src/lib/pantip/screenshot.ts`:
  - Extracts card details from `.display-post-story-wrapper .display-post-story` first.
  - Browser DOM extraction now queries `.display-post-story-wrapper .display-post-story` instead of generic `.display-post-story`.
  - HTML extraction now searches for `display-post-story-wrapper` containers only.
  - Rejects wrapper blocks that contain `display-post-wrapper-inner` or comment/reply markers.
  - Removed the broad title-nearby page scan fallback that could accidentally pick navigation text or comments.
  - Falls back to safe metadata/title text if no main topic body is found, instead of using comments.
...
### Expected behavior after deploy
- Pantip card details should come from the main topic body wrapper.
- Comments that also use `.display-post-story` should no longer be selected for card details.

---

## 2026-06-22 — Pantip comment-wrapper structural filter

### Context
- User provided a real Pantip page HTML sample for topic `Toy Story 5 (6/10) l ไม่ค่อยสนุก และยกให้เป็นภาคที่ชอบน้อยที่สุด (สปอยล์)`.
- The sample showed that both main topic posts and comments use `.display-post-story-wrapper` and `.display-post-story`.
- Comments can be distinguished structurally by `.display-post-story-wrapper.comment-wrapper`, surrounding `.section-comment`, comment IDs, and `.display-post-number` such as `ความคิดเห็นที่ ...`.
- Previous patch selected `.display-post-story-wrapper .display-post-story` but could be too strict or too broad depending on the runtime DOM.

### Changes
- Updated `src/lib/pantip/screenshot.ts` to distinguish main topic content from comments more precisely:
  - Browser DOM extraction now selects `.display-post-story-wrapper:not(.comment-wrapper) .display-post-story`.
  - DOM extraction rejects elements under `.section-comment`, `.comment-wrapper`, IDs starting with `comment-`/`reply-`, and `.display-post-wrapper-inner` containers with `.display-post-number`.
  - HTML extraction still scans `.display-post-story-wrapper`, but rejects the wrapper if its opening tag has `comment-wrapper`.
  - HTML extraction also rejects blocks containing `.display-post-number` with `ความคิดเห็นที่`.
  - Strict main-topic candidates no longer run through the broad text marker cutter, because a main topic can legitimately contain words such as `ความคิดเห็น`.
- This should keep card details from the main topic body and avoid using comment text.
...
### Validation
- Full npm validation was not completed in the sandbox because dependency install is unreliable here.
- User should run `npm install`, `npm run lint -- --no-cache`, `npx tsc --noEmit`, and `npm run build` locally.

---

## 2026-06-22 — Pantip main-post status-leftside extraction

### Context
- The previous Pantip extraction still failed for some topics and sometimes fell back to "อ่านรายละเอียดต่อได้ที่ลิงก์ต้นทาง".
- User inspected a Pantip topic and found the main topic body consistently appears after:
  - `.display-post-status-leftside`
  - `.display-post-story-wrapper`
  - `.display-post-story`
- The key difference is that this story must be inside the main post wrapper, not a comment wrapper.

### Changes
- Updated `src/lib/pantip/screenshot.ts`:
  - Adds `extractMainPostHtml()` to scope HTML extraction to `.display-post-wrapper.main-post` / `#topic-...` content.
  - Extracts the story only from `.display-post-status-leftside > .display-post-story-wrapper:not(.comment-wrapper) .display-post-story`.
  - Browser DOM extraction now uses selectors scoped to `.display-post-wrapper.main-post` and `#topic-...`.
  - Rejects comment structures using `.comment-wrapper`, `.section-comment`, `.display-post-number`, and `comment-/reply-` IDs.
  - Keeps story text intact without cutting at generic Thai words that can appear in the main story.
...
### Database / Storage / Env
- No SQL changes.
- No storage changes.
- No new environment variables.

### Validation
- Not fully run in sandbox because dependency installation is unreliable in this environment.


---

## 2026-06-22 — Pantip status-leftside direct selector

### Context
- User reported that the previous Pantip extractor still failed to find the main topic body and sometimes fell back to `อ่านรายละเอียดต่อได้ที่ลิงก์ต้นทาง`.
- User observed that the reliable main topic path appears to start at `.display-post-status-leftside`, then `.display-post-story-wrapper`, then `.display-post-story`.
- The previous implementation required `.display-post-wrapper.main-post`, which may not be present in the DOM that Puppeteer sees.

### Changes
- Updated `src/lib/pantip/screenshot.ts`:
  - Removed the `.display-post-wrapper.main-post` requirement from browser DOM selection.
  - Browser DOM extraction now uses `.display-post-status-leftside .display-post-story-wrapper:not(.comment-wrapper) .display-post-story`.
  - HTML extraction now scans every `.display-post-status-leftside` block directly, then accepts only `.display-post-story-wrapper` blocks that are not `.comment-wrapper`.
  - Keeps rejection for comment/reply structures such as `.comment-wrapper`, `.section-comment`, `id=comment-*`, and `id=reply-*`.
  - Source label changed to `status-leftside-story-wrapper` for diagnostics.
- Updated README with the current Pantip extraction rule.
...
### Validation
- Manual HTML extraction check against the user-provided Toy Story Pantip page sample found the main topic body under `.display-post-status-leftside .display-post-story-wrapper .display-post-story` and skipped comment wrappers marked `.comment-wrapper`.
- Full npm lint/typecheck/build validation was not completed in this sandbox because dependency installation is unstable here.

### Expected behavior
- Pantip card details should be pulled from the topic body under `.display-post-status-leftside` without requiring `.main-post`.
- Comment text under `.display-post-story-wrapper.comment-wrapper` should be skipped.

---

## 2026-06-22 — Pantip exact status-leftside story path

### Context
- User tested previous Pantip extraction fixes and the card still could not reliably read the main topic body.
- User identified that the main topic body appears immediately after this exact HTML path:
  - `<div class="display-post-status-leftside">`
  - `<div class="display-post-story-wrapper">`
  - `<div class="display-post-story">`
- Earlier selectors included extra assumptions such as `.main-post`, `:not(.comment-wrapper)`, comment ancestor rejection, or broad descendant matching, which could still fail against Pantip's rendered DOM.

### Changes
- Updated `src/lib/pantip/screenshot.ts`:
  - HTML extraction now looks for the exact three-level Pantip main story path with exact class values.
  - DOM extraction now finds `div.display-post-status-leftside`, then requires the direct child class to be exactly `display-post-story-wrapper`, then requires the next direct child class to be exactly `display-post-story`.
  - Removed extra reject conditions from this strict path so the extractor follows the observed DOM path directly.
  - This exact-class approach avoids comment templates such as `display-post-story-wrapper comment-wrapper`, because the wrapper class is not an exact match.
...
### Expected behavior after deploy
- Pantip card details should be extracted from the exact main topic story path when present.
- Comment wrappers with extra classes such as `comment-wrapper` should not match the exact main topic story path.
