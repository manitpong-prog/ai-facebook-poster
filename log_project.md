...
## 2026-06-22 — Pantip exact main-story HTML extractor

### Context
- User reported that some Pantip cards used comments instead of the original topic body.
- User supplied real Pantip HTML showing the main topic body under:
  - `.display-post-status-leftside`
  - `.display-post-story-wrapper`
  - `.display-post-story`
- Comments use similar `.display-post-story` markup, but their wrapper includes `comment-wrapper` and appears after the comments section.

### Changes
- Updated `src/lib/pantip/screenshot.ts`:
  - Added `extractPantipMainStoryCandidates()` for an HTML-first main topic-body extraction path.
  - Added balanced `<div>` extraction helpers so the parser can isolate the story block instead of scanning generic visible text.
  - Extracts from `.display-post-status-leftside` → `.display-post-story-wrapper` → `.display-post-story` before comments begin.
  - Skips `.display-post-story-wrapper.comment-wrapper`.
  - Removes non-body story markup such as edit history, footer/action blocks, images, iframes, and video tags before generating card text.
  - Browser extraction now captures the same main-story DOM path using desktop viewport/user agent.
  - Card excerpt selection now prioritizes `topicStoryCandidates` before metadata; generic visible text no longer competes with the main story for the card details.
  - Fallback order is now: main topic story → metadata description → safe source-link text.
...
### Expected result
- Pantip card details should use the original topic body when the supplied HTML path is present.
- Comments should not be chosen simply because they are longer or score better than the original post.

---

## 2026-07-13 — Encrypted Gemini API Key settings

### Context
- User requested changing Gemini API Keys from inside the private dashboard instead of editing Vercel for every quota-limited key.
- User also requested the ability to reveal and copy the full active key because the system is private and used by one owner.

### Changes
- Added `/dashboard/settings/ai` for saving, testing, revealing, copying, deleting, and selecting the model for Gemini.
- Added AES-256-GCM encryption using `APP_ENCRYPTION_KEY`.
- Added Workspace-specific AI configuration with environment fallback.
- Updated every Gemini generation flow to resolve the active key by Workspace.
- Updated Dashboard, Auto Pilot Diagnostics, and Deploy Readiness to understand database-backed Gemini settings.
- Added migration `0006_loose_xorn.sql` and updated Drizzle metadata.
- Updated deployment documentation and project history.

### Database / Environment
- New table: `workspace_ai_settings`.
- New required one-time environment variable: `APP_ENCRYPTION_KEY`.
- `GEMINI_API_KEY` remains supported as an optional fallback.

### Expected behavior
- After the migration and one Vercel redeploy with `APP_ENCRYPTION_KEY`, the user can replace Gemini API Keys from the dashboard with immediate effect and no further redeploy.

---

## 2026-07-13 — Gemini empty response recovery

### Context
- After replacing the Gemini API Key, manual post generation reached Gemini but returned `Gemini returned empty content`.
- The previous implementation discarded response metadata, so the UI could not distinguish a temporary empty response from safety blocking, token exhaustion, or a non-text model.

### Changes
- Updated `src/lib/ai/gemini.ts`:
  - Added shared text extraction and empty-response diagnostics.
  - Reconstructs text from candidate parts when needed.
  - Retries retryable empty responses up to three attempts.
  - Stops immediately for non-retryable safety/content finish reasons.
  - Adds response ID, finish reason, model version, safety categories, response part counts, and token counts to server diagnostics.
  - Does not log API Keys or full prompts.
  - Uses `text/plain` output and suitable output token limits.
  - Increased connection-test output from 16 to 128 tokens.
- Updated `src/lib/ai/settings.ts`:
  - Added validation to reject known non-text model families.
- Updated `src/app/dashboard/settings/ai/actions.ts`:
  - Validates the selected model before save/test.
- Updated `src/app/dashboard/settings/ai/page.tsx`:
  - Added recommended text model suggestions and a clearer warning against Image/TTS/Live/Embedding/Imagen/Veo models.
...
### Expected result
- Temporary empty Gemini responses are recovered automatically.
- Persistent failures now report an actionable reason and response ID.
- The settings page prevents saving common non-text Gemini model names.
