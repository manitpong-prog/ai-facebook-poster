...
## 2026-06-21 — Pantip multi-layer extraction and richer short captions

### Context
- User clarified that the Pantip feature must remain nearly automatic: user should paste only one Pantip topic link, then the app should extract data, generate a readable card, generate a caption, and wait for manual approval.
- Previous readable card approach was too likely to use bad Pantip shell text if metadata extraction failed. User requested the 7-step logic: accept one link, extract topic data with multiple methods, log the source, refuse to create cards from menu/footer text, and show a clear error if extraction fails.
- User also requested AI captions to avoid being too short when a topic has only a short title by adding 1-2 short summary/context sentences.

### Changes
- Updated `src/lib/pantip/screenshot.ts`:
  - Keeps the user flow link-only: no extra title/excerpt inputs added.
  - Added multi-layer topic extraction: direct HTML fetch, meta tags, title/heading, JSON script extraction, JSON-like quoted key extraction, and browser/network/DOM fallback extraction.
  - Added stricter filtering for Pantip shell text such as Download App, Certified Developer, Explore, login/signup, Pantip Pick/Trend/Mall, and similar layout/footer text.
  - If a real topic title cannot be extracted, the API now throws a clear error instead of generating a card from menu/footer text.
  - The card is still generated as a temporary readable card image and uploaded directly to Facebook only after manual approval.
- Updated `src/lib/ai/gemini.ts`:
  - Pantip captions now allow about 80-90 words instead of being extremely short.
  - Prompt asks Gemini to use title/excerpt as the base and add 1-2 short human-sounding summary/context sentences.
  - Fallback caption now also adds one short context sentence if the extracted content is too short.
- Updated `src/app/dashboard/pantip/pantip-source-client.tsx`:
  - Default per-post style instruction now asks for a short human caption with 1-2 summary/context sentences.
...
- No database schema changes.
- No storage added.
- Preview images remain temporary only.

### Environment Variables
- No new environment variables.

### Expected behavior after deploy
- User still only pastes a Pantip topic link.
- If real topic metadata is found, the system creates a readable card and a richer short caption.
- If real topic metadata is not found, the system shows a clear error and does not create a misleading card from Pantip menu/footer text.
